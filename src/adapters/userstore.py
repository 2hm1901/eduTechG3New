...
class SQLiteUserStore:
    ...
    def _init_schema(self):
        self.conn.executescript("""
            ...
            CREATE INDEX IF NOT EXISTS quiz_results_user_idx ON quiz_results(user_id, created_at DESC);
    ...
    def update_doc_folder(self, user_id: str, doc_id: str, folder_name: str) -> None:
        ...
        if row:
            ...
            metadata["folders"] = folders
            self.conn.execute(
                "UPDATE user_docs SET metadata = ? WHERE user_id = ? AND doc_id = ?",
                (json.dumps(metadata), user_id, doc_id)
            )
            self.conn.commit()
    ...
    def log_quiz_result(self, user_id: str, doc_id: str, score: int, total: int, details: dict | None = None) -> None:
        percentage = round(score / total * 100) if total else 0
        self.conn.execute(
            "INSERT INTO quiz_results (user_id, doc_id, score, total, percentage, details) VALUES (?, ?, ?, ?, ?, ?)",
            (user_id, doc_id, score, total, percentage, json.dumps(details or {})),
        )
        self.conn.commit()
    ...
    def get_dashboard_stats(self, user_id: str) -> dict:
        ...
        queries = self.recent_queries(user_id, limit=50)
        cur = self.conn.execute(
            "SELECT doc_id, score, total, percentage, created_at FROM quiz_results WHERE user_id = ? ORDER BY created_at DESC",
            (user_id,),
        )
        quizzes = [{"doc_id": r[0], "score": r[1], "total": r[2], "percentage": r[3], "created_at": r[4]} for r in cur.fetchall()]
        avg_score = 0
        if quizzes:
            avg_score = round(sum(q["percentage"] for q in quizzes) / len(quizzes))
        # Build combined recent activity timeline
        recent_activity = []
        for d in docs[:5]:
            recent_activity.append({"type": "upload", "title": d.get("filename", d.get("doc_id", "")), "time": d.get("created_at", "")})
        for q in queries[:5]:
            recent_activity.append({"type": "query", "title": q.get("query", "")[:80], "time": q.get("created_at", "")})
        for qz in quizzes[:5]:
            recent_activity.append({"type": "quiz", "title": f"Quiz score: {qz['score']}/{qz['total']}", "time": qz.get("created_at", "")})
        recent_activity.sort(key=lambda x: x.get("time", ""), reverse=True)
        return {
            "user_id": user_id,
            "total_docs": len(docs),
    ...
    def authenticate_user(self, username: str, password: str):
        ...
        if not row:
            return None
        hashed, _ = _hash_password(password, row[2])
        if hashed != row[1]:
            return None
        return {"user_id": row[0], "username": row[0]}
