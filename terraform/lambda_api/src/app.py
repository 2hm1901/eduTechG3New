...
@app.post("/api/bank/documents/upload/presign")
@app.post("/upload-pdf")
def api_presign_upload(req: PresignRequest, request: Request, x_user_id: str | None = Header(default=None)) -> dict:
    return handlers.handle_presign(
        user_id=_resolve_user_id(request, x_user_id),
...
@app.post("/api/bank/documents/upload/finalize")
def api_finalize_upload(req: FinalizeRequest, request: Request, x_user_id: str | None = Header(default=None)) -> dict:
    return handlers.handle_finalize(
        user_id=_resolve_user_id(request, x_user_id),
        doc_id=req.docId,
        filename=req.filename,
        key=req.key,
...
@app.put("/api/bank/documents/upload/direct")
async def api_direct_upload(request: Request, key: str) -> dict:
    """Local-only direct upload to simulate S3 PUT."""
    data = await request.body()
    return handlers.handle_direct_upload(key, data, storage)
